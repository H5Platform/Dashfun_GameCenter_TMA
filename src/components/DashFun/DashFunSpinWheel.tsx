...
import {
  ...
  SpinWheelData,
  SpinWheelReward,
} from "../DashFunData/SpinWheelData";

export const useDashFunSpinWheel = (): [
  spinwheel: SpinWheelData,
  spin: () => Promise<SpinWheelData | null>,
  claim: () => Promise<SpinWheelReward | null>
] => {
  ...
  const getSpinwheel = async () => {
    ...
    const sw = await SpinWheelApi.getInfo(
      initDataRaw as string as string,
      game?.id as string
    );
  ...
  const spin = async (): Promise<SpinWheelData | null> => {
    if (
      ...
      initData == null ||
      spinwheel == null ||
      spinwheel.canSpin() == false
    )
      return null;

    const r = await SpinWheelApi.spin(
      initDataRaw as string as string,
      game?.id as string
    );
  ...
  const claim = async (): Promise<SpinWheelReward | null> => {
    if (
      ...
      initData == null ||
      spinwheel == null ||
      spinwheel.canClaim() == false
    )
      return null;

    const r = await SpinWheelApi.claim(
      initDataRaw as string as string,
      game?.id as string
    );
